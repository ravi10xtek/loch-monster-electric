          ...
          <div className="scard" key={card.label}>
            ...
            {subs.length > 0 && (
              <ul className="scard-pills" aria-label={`${card.label} services`}>
                {subs.map((s) => (
                  <li key={s.href}>
                    <a className="scard-pill" href={s.href}>{s.label}</a>
                  </li>
                ))}
              </ul>
            )}
...
export default async function Services({ cardImages: cardImagesProp, subServicesByHub: subProp } = {}) {
  // Accept pre-fetched data from server pages (avoids re-fetch loops when
  // this component is rendered inside live-preview client trees).
  const [cardImages, subServicesByHub] = await Promise.all([
    cardImagesProp !== undefined ? Promise.resolve(cardImagesProp) : getCategoryHubCardImages(),
    subProp !== undefined ? Promise.resolve(subProp) : getCategoryHubSubServices(),
  ])
  return (
